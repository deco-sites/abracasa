...
function SuggestionCard(
  { product, preload, itemListName, hasCheckbox }: Props,
) {
  const {
    url,
    productID,
  ...
  const { price: definitivePrice, listPrice: definitiveListPrice } = useOffer(
    offers,
  );
  ...
  return (
    <>
      ...
      {!isDesktop && (
        <div
          id={id}
          class="flex xl:hidden items-center justify-center gap-6 relative p-2 border-black/80 group w-full"
          ...
          {hasCheckbox && (
            <div class="-translate-y-16 right-0 z-30 absolute">
              <input
                ...
                onClick={() => hasCheckbox.setIsChecked((prev) => !prev)}
                class="w-6 h-6 rounded-md border border-blue-base checked:bg-blue-base appearance-none custom-checkbox"
              />
            </div>
          )}
      ...
      {isDesktop && (
        <div
          id={id}
          class="hidden xl:block card card-compact justify-between p-2 pb-3 border-black/80 group max-w-[220px]"
          data-deco="view-product"
        >
          <SendEventOnClick
            id={id}
