...
export interface StatProps {
  items: Array<{ label: string; title: string; subtitle: string }>;
  align: "start" | "center";
}
...
export interface Props {
  ...
  faq?: {
    title?: string;
    subtitle?: string;
    "q&a"?: Array<{ question: string; answer: string }>;
  };
}
  ...
  return (
    <div class="stats stats-vertical lg:stats-horizontal shadow">
      {items?.map((item) => (
        <div
          class={`stat ${
            align === "center" ? "place-items-center" : "place-items-start"
          }`}
        >
...
function FAQ({ faq }: Pick<Props, "faq">) {
  if (!faq || faq["q&a"] && faq["q&a"].length == 0) return null;
...
export default function Cashback(
  { title, bgColor, stats, sections, faq }: Props,
) {
  return (
    <div class="flex flex-col max-w-[1180px] mx-auto gap-2">
      <div
        style={{ background: bgColor }}
        class="flex flex-col w-full gap-3 py-10 px-6"
      >
