...
function HeroBannerGrid({ title, mainImage, imageItems }: Props) {
    return (
        <div class="mb-10 lg:mx-auto lg:max-w-[83%] ">
            <span class="font-sans text-2xl text-dimgray mb-5 px-4 lg:px-0 lg:mb-[14px] block">{title}</span>
            <div class="flex flex-col lg:flex-row">
                <a href={mainImage?.href}>  <Image
                ...
                <ul class="grid grid-cols-2 grid-rows-3 gap-x-5 gap-y-4 lg:grid-cols-3 2xl:grid-cols-5 lg:grid-rows-2 lg:gap-x-[14px] lg:gap-y-3 px-5 mt-[18px] lg:mt-0">
                    {imageItems?.map((img) => (
                        <li class="sm:flex justify-center">              <a href={img?.href}>  <Image
                            src={img?.srcMobile ?? ''}
                            alt={img?.alt ?? ''}
